...
void show_message(VMSTR text){
	...
	VMWCHAR cZZZQ[4] = {};
        ...
        VMINT hZZZ = 0;      //string "WWp" height --16
        VMINT xBaseline = 0; //string "WWp" baseline --2
	...
	vm_graphic_setcolor(&color);
        ...
        vm_ascii_to_ucs2(cZZZQ, (strlen("WWp") + 1) * 2, "WWp");  //Š???
        xBaseline = vm_graphic_get_string_baseline(cZZZQ);
        hZZZ = vm_graphic_get_string_height(cZZZQ);

        //wText = vm_malloc(((text_lengt + 1) * sizeof(VMWCHAR));
	...
	while(*p)
	{
		...
		szTemp[i++] = *p; //is simboliu konstruojame USC2 eilute szTemp
		szLine[0] = *p;
		szLine[1] = 0;
